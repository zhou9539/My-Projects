...
class shixiseng_crawler():
    def __init__(self, city, keyword, publishTime, pages):
        db = pymysql.connect(host="你的host",database="你新建的数据库名",user="你的用户名",password="你的密码",port=3306,charset='utf8')  #新建的数据库必须叫'shixiseng’, 问就是我懒得改了^_^
        self.city = city
    ...
    def run(self):
        ...
        for i in range(1,self.pages):
            #获取职位列表URL中的网页数据
            req = requests.get(f'https://www.shixiseng.com/interns?keyword={self.keyword}&page={i}&city={self.city}&type=intern&publishTime={self.publishTime}',
    			headers = headers)
            html = req.text
        ...
        try:
            ...
            try:
                ...
                sqlcmd = "insert into shixiseng \
                          (positionName,salary,company,area,education,dayPerWeek,duration,jobDescription,companyDetails,url,uploadTime) \
                          values ('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s')" \
                          % (positionName,salary,company,area,education,dayPerWeek,duration,jobDescription,companyDetails,url,datetime.datetime.now())
    ...
    def write_csv(self, data):
        with open('./shixiseng_test.csv', 'a', encoding='utf-8', newline='') as f:
            #引入csv
            writer = csv.writer(f)
            #写入csv
